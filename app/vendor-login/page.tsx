  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!querySnapshot.empty) {
        ...
        // 3. අසාර්ථක නම් Alert එකක් පමණක් පෙන්වීම (No bounce, no red border)
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col justify-center py-12 px-6 font-sans">
      <div className="sm:mx-auto sm:w-full sm:max-w-md bg-white p-10 rounded-[2.5rem] shadow-2xl border-b-8 border-orange-600">
        <div className="text-center mb-8">
           <h2 className="text-3xl font-black italic tracking-tighter text-zinc-900 uppercase leading-none">VENDOR LOGIN</h2>
        ...
        <form className="space-y-4" onSubmit={handleLogin}>
          ...
          <button 
            type="submit" 
            disabled={isLoading} 
            className="w-full bg-orange-600 text-white font-black py-5 rounded-3xl shadow-lg hover:bg-zinc-900 transition-all uppercase tracking-widest text-xs disabled:opacity-50"
          >

...
export default function DailyReport() {
  ...
  useEffect(() => {
    const fetchTodayOrders = async () => {
      try {
        // asc දැම්මම මුලින්ම ආපු ඕඩරේ උඩටත්, අලුත්ම එක යටටත් එනවා (A-Z Sort)
    ...
    <div className="min-h-screen bg-white p-4 md:p-10 font-sans text-black">
      <div className="max-w-6xl mx-auto border-2 border-black p-6">
                ...
                orders.map((order, index) => (
                  <tr key={index} className="hover:bg-gray-50 border-b border-black/10">
                    ...
                    <td className="border-r border-black p-2 text-sm">
                      ...
                      <div className="text-[11px] text-blue-600 font-bold mt-1 italic border-t border-dashed border-gray-300 pt-1">+ Delivery Charge</div>
                    </td>
                    ...
                    <td className="border-r border-black p-2 text-center font-black text-xl">{order.paymentMethod === 'COD' ? '✔' : '-'}</td>
                    <td className="border-r border-black p-2 text-center font-black text-xl">{order.paymentMethod === 'Online' ? '✔' : '-'}</td>
                    <td className="p-2 text-right font-black bg-gray-50">{order.totalAmount?.toFixed(2)}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        <div className="mt-10 text-center no-print">
          <button onClick={() => window.print()} className="bg-black text-white px-10 py-4 font-black uppercase tracking-widest hover:bg-gray-800 transition shadow-xl active:scale-95">

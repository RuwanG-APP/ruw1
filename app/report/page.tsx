  ...
  if (!isAuth) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-black">
        <input type="password" autoFocus onKeyDown={(e)=> (e.target as any).value === 'ruwan123' && e.key === 'Enter' && setIsAuth(true)} placeholder="ADMIN PIN" className="p-4 rounded-2xl text-center outline-none font-black text-lg" />
      </div>
    );
  }
        ...
        <div className="overflow-x-auto mb-10 border-4 border-black">
          <table className="w-full text-sm">
            <thead className="bg-black text-white font-black uppercase">
              <tr><th className="p-4">ID</th><th className="p-4 text-left">Customer & Items</th><th className="p-4 text-right">Total</th><th className="p-4">Manage</th></tr>
            </thead>
            <tbody>
              {orders.map((o, i) => (
                <tr key={i} className="border-b-4 border-black">
                  ...
                  <td className="p-3 uppercase border-r-2 border-black">
                    ...
                    <div className="text-[10px] text-zinc-500 font-bold">{o.items.map((it:any)=>`${it.qty}x ${it.name}`).join(', ')}</div>
                  </td>
                  <td className="p-3 text-right font-black bg-zinc-50 border-r-2 border-black text-lg text-black">Rs. {o.totalAmount}.00</td>
                  <td className="p-3">
          ...
          <div className="border-4 border-black p-6 bg-zinc-50 relative shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
            ...
            <div className="space-y-3 mt-4">
                ...
                <div key={name} className="flex justify-between border-b-2 border-dashed border-zinc-200 pb-1 font-black uppercase text-xs">
                    <span className="text-zinc-500">{name}</span><span className="text-green-700">Rs. {profit.toFixed(2)}</span>
                </div>
                ))}
            </div>
          </div>
          <div className="space-y-4">
             <div className="border-4 border-black p-5 bg-blue-50 font-black flex justify-between uppercase italic shadow-[8px_8px_0px_0px_rgba(30,64,175,1)]"><span>Delivery:</span><span className="text-blue-800 text-black">Rs. {breakdown.totalDelivery.toFixed(2)}</span></div>
             <div className="border-4 border-black p-5 bg-green-50 font-black flex justify-between uppercase italic shadow-[8px_8px_0px_0px_rgba(21,128,61,1)]"><span>Total Profit:</span><span className="text-green-700">Rs. {totalNetProfit.toFixed(2)}</span></div>

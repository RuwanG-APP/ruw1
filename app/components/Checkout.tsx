...
export default function Checkout({ cartItems: rawCartItems, subTotal, goBack, lang, clearCart }: any) {
  ...
  const [walletBalance, setWalletBalance] = useState(0); // Wallet සල්ලි
  const [isProcessing, setIsProcessing] = useState(false);

  const deliveryFee = isCityLimit ? 150 : 250;
  const finalTotal = subTotal + deliveryFee;

  // අලුතින්: Wallet එකේ සල්ලි තියෙනවද කියලා චෙක් කරනවා
  ...
  // Wallet එකෙන් ගෙවන ගාණ සහ අතින් ගෙවන්න ඕනේ ගාණ
  ...
  const handleConfirmOrder = async () => {
    ...
    try {
      ...
      await addDoc(collection(db, "orders"), {
        ...
        walletUsed: usedWalletAmount, // කොච්චරක් Wallet එකෙන් කැපුණද
        totalAmount: amountToPay,     // අතින් ගෙවපු ගාණ
        createdAt: serverTimestamp(),
        status: (paymentMethod === 'Online' && amountToPay > 0) ? "Paid (Pending Gateway)" : "New"
      });

      // Wallet එකෙන් සල්ලි පාවිච්චි කළා නම් ඒක අඩු කරනවා
    ...
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 backdrop-blur-sm">
      <div className="bg-white p-6 sm:p-8 rounded-3xl shadow-2xl w-full max-w-lg border animate-fade-in relative max-h-[90vh] overflow-y-auto">
        ...
        <form className="space-y-4 mt-4">
          ...
          <div>
            <input type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} onBlur={checkWalletBalance} disabled={isProcessing} className="w-full border-2 border-gray-600 rounded-xl p-2.5 focus:border-orange-500 focus:outline-none disabled:bg-gray-100" placeholder="Phone (Click outside to check wallet)" />
          </div>
        ...
        <div className="mt-6 bg-gray-50 p-4 rounded-xl border border-gray-100">
          ...
          {/* අලුතින්: Wallet Balance එක අඩු කරන තැන */}
          {walletBalance > 0 && (
